...
namespace ARCeye.VOT
{
    public class ImageUtility
    {
        ...
        static private Texture2D m_RequestingTexture = null;
        static private RenderTexture m_RenderTexture = null;
        static private Material m_PreviewRotater;

        static public byte[] ConvertToJpegData(Texture texture)
        {
        ...
        static public void ConvertRenderTextureToTexture2D(RenderTexture renderTexture, ref Texture2D targetTexture)
        {
            ...
            {
                targetTexture = new Texture2D(renderTexture.width, renderTexture.height, TextureFormat.RGBA32, false);
            }
        ...
        static public Texture2D RotateTexture(Texture originalTexture, Matrix4x4 texMatrix)
        {
            ...
            {
                Shader shader = Shader.Find("VOTSDK/PreviewRotation");
                if (shader == null)
                {
                    Debug.LogError("[ImageUtility] Shader 'VOTSDK/PreviewRotation'를 찾을 수 없음");
                    return null;
                }
